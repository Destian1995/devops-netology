...
Так же отмечаю что качество отработки скрипта полной установки зависит от качества сети(скорость\стабильность). При развертывании случались проблемы с тем что terraform мог не прокинуть ключи ssh на хосты. Либо один из серверов был недоступен во время конфигурирования кластера(по необъяснимым причинам Ansible то устанавливался правильно то "как попало", причем если я выполняю команды скрипта отдельно то все устанавливается корректно, затем при повторном запуске скрипта все устанавливается снова корректно, вообщем мистика). 
Однако при перезапуске скрипта(предварительно снеся инфраструктуру) все отработывало штатно(как повезет со стабильностью сети VPN)
Вообщем любые проблемы(за исключением окружения потому что у меня оно устанавливалось по разному) это сугубо сетевые проблемы.
