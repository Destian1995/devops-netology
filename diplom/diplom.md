...
Так же отмечаю что качество отработки скрипта полной установки зависит от качество сети(скорость\стабильность). При развертывании случались проблемы с тем что terraform мог не прокинуть ключи ssh на хосты. Либо один из серверов был недоступен во время конфигурирования кластера. Однако при перезапуске скрипта(предварительно снеся инфраструктуру) все отработавало штатно(как повезет со стабильностью сети VPN)
