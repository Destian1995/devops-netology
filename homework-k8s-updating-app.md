...
* Если недоступность приложения недопустима, то я бы выбрал стратегию Canary update, так как можно развернуть одну или несколько реплик (точное количество у нас не указано), используя доступные ресурсы в 20%, но при 
  этом надо перенаправлять трафик частично, чтобы избежать перегрузок и прерывания сервиса. Постепенно уменшаем количество старых реплик на новые и постепенно переводим на новые реплики трафик по какому-нибудь 
  признаку.
