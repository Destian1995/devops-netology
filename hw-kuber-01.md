...
Кратко: развернул на ВМ vagrant microk8s. Но с самого компьютера подключится к сайту дашборда который развернут на ВМ не удается по причине ERR_CONNECTION_TIMED_OUT.
--
...
Но после ввода команды kubectl proxy на сайт все равно не зайти по той же причине...

( ERR_CONNECTION_TIMED_OUT )

Пробрасывал еще порты в VirtualDub на конкретной машинке, но все тщетно.
